...
\item Compare performance to simple RNN, FFNN, and linear reference models
\end{itemize}
...
\frame{
\frametitle{Virtual Trading Points}
\begin{figure}
\includegraphics[width=0.8\textwidth]{\string"../../Plots/hub_map\string".png}
\end{figure}

}

\frame{
\frametitle{Price History TTF Front Month}
\begin{figure}
\includegraphics[width=0.8\textwidth]{\string"../../Plots/ttf_fm_level\string".png}
\end{figure}
...
\item $\frac{h_t}{dW_{ij}} = \frac{df(z)}{dz}(\frac{dW}{dW_{ij}}h_{t-1} + W\frac{dh_{t-1}}{dW_{ij}})$
\item $\frac{h_t}{dW_{ij}} = \frac{df(z)}{dz}(\frac{dW}{dW_{ij}}h_{t-1} + W\frac{df(z)}{dz}(\frac{dW}{dW_{ij}}h_{t-2} + W\frac{dh_{t-2}}{dW_{ij}})$
\end{itemize}
\end{itemize}
}
...
\frametitle{Vanishing Gradient Problem}
\begin{itemize}
\item Contribution of $\frac{dh_{t-k}}{dW_{ij}}$ is multiplied by $(\frac{df(z)}{dz})^kW^{k-1}$ \begin{itemize}
...
\begin{itemize}
\item $h_t = \sigma_{o}(W_4h_{t-1} + U_4x_t)*f_o(c_t)$
\item $c_t = \sigma_{f}(W_1h_{t-1} + U_1x_t)*c_{t-1} +  \sigma_{i}(W_2h_{t-1} + U_2x_t)*f_i(W_3h_{t-1} + U_3x_t)$
\end{itemize}
...
\item \textbf{Energy Commodity Prices:} Target variable, other gas futures, Brent Oil Futures, Coal Futures, Electricity Base / Peak futures
...
\item Linear Reference Model: AR(1) \begin{itemize}
\item Parameter estimate very close to 1, predictions almost identical to current price value
\end{itemize}
\end{itemize}
}
